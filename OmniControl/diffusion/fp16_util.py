...
class MixedPrecisionTrainer:
    ...
    def _optimize_fp16(self, opt: th.optim.Optimizer):
        ...
        # Scale all gradients (not just the first one)
        for param in self.master_params:
            if param.grad is not None:
                param.grad.mul_(1.0 / (2 ** self.lg_loss_scale))
        
        # Apply gradient clipping if enabled (after scaling all gradients)
        if self.grad_clip > 0:
            th.nn.utils.clip_grad_norm_(self.master_params, self.grad_clip)
            grad_norm_clipped, _ = self._compute_norms()
            logger.logkv_mean("grad_norm_clipped", grad_norm_clipped)
    ...
    def _optimize_normal(self, opt: th.optim.Optimizer):
        ...
        if self.grad_clip > 0:
            ...
            th.nn.utils.clip_grad_norm_(self.master_params, self.grad_clip)
            # Recompute grad norm after clipping for logging
            grad_norm_clipped, _ = self._compute_norms()
            logger.logkv_mean("grad_norm_clipped", grad_norm_clipped)
