...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.l1_loss = lambda a, b: (a - b).abs()

        if dataset == 'humanml':
            if data_rep == 'smpl':
                spatial_norm_path = './dataset/HumanML3D_amass'
                data_root = './dataset/HumanML3D_amass'
            else:
        ...
        self.joints_value_from = joints_value_from  # 'joints' or 'smpl'
        
        if self.joints_value_from == 'smpl':
    ...
    def gradients(self, x, hint, mask_hint, joint_ids=None):
        with torch.enable_grad():
            ...
            x_ = x_.squeeze(2)
            x_ = x_ * self.std + self.mean # (bs, seqlen, feat_dim)
            if self.data_rep == 'smpl':
                n_joints = 24
                if self.joints_value_from == 'joints':
                    joints = extract_joints_batch(x_, 
                                    "smplrifke",
                                    fps=20,
                                    value_from="joints",
                                    smpl_layer=None,keep_torch=True)
                else:
                    joints = extract_joints_batch(x_,
                                    "smplrifke",
                                    fps=20,
                                    value_from="smpl",
                                    smpl_layer=self.smplh_dict['neutral'].to(x_.device),
                                    keep_torch=True, joints_only=True)
                joint_pos = joints["joints"]
            else:
    ...
    def guide(self, x, t, model_kwargs=None, t_stopgrad=-10, scale=.5, n_guide_steps=10, train=False, min_variance=0.01):
        ...
        # Adjust guide steps based on sampler type
        # For DDIM with fewer steps, we need to adjust the threshold
        if self.sampler == 'ddim':
            # DDIM typically uses fewer steps (e.g., 50 steps instead of 1000)
            # The timesteps are spaced out, so we need higher thresholds
            if train:
                # For DDIM, most timesteps will be > 20, so we use a higher threshold
                if t[0] < 100:  # Roughly equivalent to t < 20 in DDPM
                    n_guide_steps = 100
                else:
                    n_guide_steps = 20
            else:
                # For inference with DDIM
                if t[0] < 20:  # Roughly equivalent to t < 10 in DDPM, speed up
                    n_guide_steps = 500
                else:
                    n_guide_steps = 10
        else:
    ...
    def ddim_sample(
        ...
        :param model_kwargs: if not None, a dict of extra keyword arguments to
            pass to the model. This can be used for conditioning.
        :param eta: the eta parameter for DDIM. If None, uses self.ddim_eta.
        :param const_noise: If True, will use the same noise for all samples in the batch.
        :return: a dict containing the following keys:
                 - 'sample': a random sample from the model.
                 - 'pred_xstart': a prediction of x_0.
        """
        ...
        if 'hint' in model_kwargs['y'].keys():
            # spatial guidance/classifier guidance
            # For DDIM, we apply guidance to pred_xstart instead of mean
            out["pred_xstart"] = self.guide(out["pred_xstart"], t, model_kwargs=model_kwargs)
    ...
    def p_sample_loop_progressive(
        ...
        for i in indices:
            ...
            with th.no_grad():
                # Choose sampling function based on sampler type
                if self.sampler == 'ddim':
    ...
    def training_losses_dpo(self, model, ref_model, x_start, t, model_kwargs=None,
                            noise=None, dataset=None, sft_scale=0., beta_dpo=5000.,
                            original_motion=None, metrics_ratio=1.0,
                            training_step=0,
                            dpo_loss_scale=1.0, dpo_loss_type='dpo'):
        """
        Compute training losses for a single timestep.
        ...
        if noise is None:
            noise = th.randn_like(x_start)
        # remove this to use different noise for winner and loser (for DDIM sampling)
        noise_w, _ = noise.chunk(2)
        ...
        # Generate CFG masks once for both model and ref_model
        # Each branch (CMDM and MDM) gets its own mask, but model and ref_model share the same masks
        # For DPO: winner and loser should also share the same masks for fair comparison
        cmdm_text_mask = None
        mdm_text_mask = None
        if model.training and hasattr(model, 'cond_mask_prob') and model.cond_mask_prob > 0.:
            bs = x_t.shape[0]
            # For DPO training, bs includes both winner and loser (bs = batch_size * 2)
            # We generate masks for half batch size and repeat them
            actual_batch_size = bs // 2 if bs % 2 == 0 and bs > 1 else bs
            
            # Generate masks for one set (will be shared by winner and loser)
            cmdm_mask_single = torch.bernoulli(torch.ones(actual_batch_size, device=x_t.device) * model.cond_mask_prob).view(actual_batch_size, 1)
            mdm_mask_single = torch.bernoulli(torch.ones(actual_batch_size, device=x_t.device) * model.cond_mask_prob).view(actual_batch_size, 1)
            
            # Repeat masks for both winner and loser if DPO
            if bs != actual_batch_size:
                cmdm_text_mask = cmdm_mask_single.repeat(2, 1)
                mdm_text_mask = mdm_mask_single.repeat(2, 1)
            else:
                cmdm_text_mask = cmdm_mask_single
                mdm_text_mask = mdm_mask_single
        
        # Generate a random seed for this batch to ensure model and ref_model use the same randomness
        # but different batches have different random states
        batch_seed = int(training_step)
        
        # Forward pass with shared masks (between model and ref_model, but different for CMDM and MDM)
        torch.manual_seed(batch_seed)
        model_output = model(x_t, self._scale_timesteps(t), y=model_kwargs['y'], 
                            cmdm_text_mask=cmdm_text_mask, mdm_text_mask=mdm_text_mask)
        ...
        if original_motion is not None:
            # add SFT loss
            sft_loss = self.masked_l2(original_motion, model_output, mask)
            sft_loss_w, sft_loss_l = sft_loss.chunk(2)
            # raw_sft_loss = 0.5 * (sft_loss_w + sft_loss_l)
            terms['sft_loss_gt_w'] = sft_loss_w
            terms['sft_loss_gt_l'] = sft_loss_l
            
        model_losses = self.masked_l2(target, model_output, mask) # mean_flat(rot_mse)
        model_losses_w, model_losses_l = model_losses.chunk(2)
        model_diff = model_losses_w - model_losses_l # These are both LBS (as is t)
        
        with torch.no_grad(): # Get the reference policy (unet) prediction
            # Use the SAME seed for ref_model to ensure fair comparison
            torch.manual_seed(batch_seed)
            ref_pred = ref_model(x_t, self._scale_timesteps(t), y=model_kwargs['y'],
                               cmdm_text_mask=cmdm_text_mask, mdm_text_mask=mdm_text_mask).detach()
            ref_losses = self.masked_l2(target, ref_pred, mask)
            ref_losses_w, ref_losses_l = ref_losses.chunk(2)
            ref_diff = ref_losses_w - ref_losses_l
        
        terms["rot_mse_w_diff"] = model_losses_w - ref_losses_w
        terms["rot_mse_l_diff"] = model_losses_l - ref_losses_l

        dpo_weights = 1.0

        z = model_diff - ref_diff
        beta_value = beta_dpo
        scale_term = -0.5 * beta_value
        inside_term = scale_term * z

        loss_type = dpo_loss_type.lower() if isinstance(dpo_loss_type, str) else 'dpo'
        if loss_type == 'ipo':
            h = -0.5 * z
            target = 0.5 * beta_value
            dpo_loss = (h - target) ** 2
        else:
            dpo_loss = -1 * F.logsigmoid(inside_term)
        terms['dpo_loss'] = dpo_loss

        terms['sft_loss'] = model_losses_w
        if sft_scale > 0.:
            loss += sft_scale * model_losses_w

        loss += dpo_weights * dpo_loss * metrics_ratio * dpo_loss_scale
        terms["loss"] = loss

        terms["win_rate"] = (inside_term > 0).float()
