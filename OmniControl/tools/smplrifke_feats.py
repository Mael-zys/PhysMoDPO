...
def ungroup(features: Tensor) -> tuple:
    assert features.shape[-1] == 205
    (
        root_grav_axis,
        vel_trajectory_local,
...
def smplrifkefeats_to_smpldata_batch(features: Tensor, first_angle=0.0):
    """
    Batch version of smplrifkefeats_to_smpldata.
    
    Args:
        features: Tensor of shape (B, T, 205) where B is batch size, T is sequence length
        first_angle: Initial angle for each sequence (scalar or tensor of shape (B,))
    
    Returns:
        smpldata: Dictionary with 'poses', 'trans', 'joints' each having batch dimension
    """
    # Check if input has batch dimension
    if len(features.shape) == 2:
        # Single sequence, use original function
        return smplrifkefeats_to_smpldata(features, first_angle=first_angle)
    
    assert len(features.shape) == 3, f"Expected 3D input (B, T, 205), got {features.shape}"
    assert features.shape[-1] == 205, f"Expected last dim to be 205, got {features.shape[-1]}"
    
    batch_size = features.shape[0]
    
    # Handle first_angle as scalar or tensor
    if isinstance(first_angle, (int, float)):
        first_angle = torch.full((batch_size,), first_angle, 
                                 dtype=features.dtype, device=features.device)
    else:
        first_angle = first_angle.to(features.device)
    
    # Ungroup features for all batches at once
    (
        root_grav_axis,      # (B, T)
        vel_trajectory_local, # (B, T, 2)
        vel_angles,          # (B, T)
        poses_local_flatten, # (B, T, 132)
        joints_local_flatten # (B, T, 69)
    ) = einops.unpack(features, [[], [2], [], [132], [69]], "b k *")
    
    poses_local = einops.rearrange(poses_local_flatten, "b k (l t) -> b k l t", t=6)  # (B, T, 22, 6)
    joints_local = einops.rearrange(joints_local_flatten, "b k (l t) -> b k l t", t=3)  # (B, T, 23, 3)
    
    # Convert 6D rotation to matrix
    poses_mat_local = rotation_6d_to_matrix(poses_local)  # (B, T, 22, 3, 3)
    global_orient_local = poses_mat_local[:, :, 0]  # (B, T, 3, 3)
    
    # Remove the dummy last angle and integrate the angles
    angles = torch.cumsum(vel_angles[:, :-1], dim=1)  # (B, T-1)
    # The first angle is the provided first_angle for each batch
    angles = first_angle[:, None] + torch.cat((0 * angles[:, [0]], angles), dim=1)  # (B, T)
    
    # Construct the rotation matrix for all batches
    rotZ = axis_angle_rotation("Z", angles)  # (B, T, 3, 3)
    
    # Rotate the trajectory (normal rotation in the indexes)
    vel_trajectory = torch.einsum("btjk,btk->btj", rotZ[:, :, :2, :2], vel_trajectory_local)  # (B, T, 2)
    
    # Rotate joints
    joints = torch.einsum("btjk,btlk->btlj", rotZ[:, :, :2, :2], joints_local[..., [0, 1]])  # (B, T, 23, 2)
    joints = torch.stack(
        (joints[..., 0], joints[..., 1], joints_local[..., 2]), axis=-1
    )  # (B, T, 23, 3)
    
    # Remove the dummy last velocity and integrate the trajectory
    trajectory = torch.cumsum(vel_trajectory[..., :-1, :], dim=-2)  # (B, T-1, 2)
    # The first position is zero
    trajectory = torch.cat((0 * trajectory[..., [0], :], trajectory), dim=-2)  # (B, T, 2)
    
    # Add the pelvis (which is still zero)
    joints = torch.cat((0 * joints[:, :, [0]], joints), axis=2)  # (B, T, 24, 3)
    ...
    # Get back the translation
    trans = torch.cat([trajectory, root_grav_axis[..., None]], dim=-1)  # (B, T, 3)
    
    # Remove the predicted Z rotation inside global_orient_local
    global_euler_local = matrix_to_euler_angles(global_orient_local, "ZYX")  # (B, T, 3)
    _, rotY_angle, rotX_angle = torch.unbind(global_euler_local, -1)
    rotY = axis_angle_rotation("Y", rotY_angle)  # (B, T, 3, 3)
    rotX = axis_angle_rotation("X", rotX_angle)  # (B, T, 3, 3)
    
    # Replace it with the one computed with velocities
    global_orient = rotZ @ rotY @ rotX  # (B, T, 3, 3)
    poses_mat = torch.cat(
        (global_orient[..., None, :, :], poses_mat_local[..., 1:, :, :]), dim=-3
    )  # (B, T, 22, 3, 3)
    
    poses = matrix_to_axis_angle(poses_mat)  # (B, T, 22, 3)
    # flatten back
    poses = einops.rearrange(poses, "b k l t -> b k (l t)")  # (B, T, 66)
