...
class CMDM(torch.nn.Module):
    ...
    def mask_cond(self, cond, force_mask=False, mask=None):
        """
        Apply CFG masking to condition.
        
        Args:
            cond: condition tensor [bs, d]
            force_mask: if True, return zeros
            mask: pre-generated mask [bs, 1]. If None, generate new mask
        ...
        if force_mask:
            ...
            if mask is None:
                # Generate new mask if not provided
    ...
    def cmdm_forward(self, x, timesteps, y=None, weight=1.0, text_mask=None):
        """
        Realism Guidance
        x: [batch_size, njoints, nfeats, max_frames], denoted x_t in the paper
        timesteps: [batch_size] (int)
        text_mask: optional pre-generated CFG mask for text [bs, 1]
        """
        ...
        if 'text' in self.cond_mode:
            ...
            emb += self.c_embed_text(self.mask_cond(enc_text, force_mask=force_mask, mask=text_mask))
    ...
    def mdm_forward(self, x, timesteps, y=None, control=None, text_mask=None):
        """
        x: [batch_size, njoints, nfeats, max_frames], denoted x_t in the paper
        timesteps: [batch_size] (int)
        text_mask: optional pre-generated CFG mask for text [bs, 1]
        ...
        if 'text' in self.cond_mode:
            enc_text = self.encode_text(y['text'])
            emb += self.embed_text(self.mask_cond(enc_text, force_mask=force_mask, mask=text_mask))
    ...
    def forward(self, x, timesteps, y=None, cmdm_text_mask=None, mdm_text_mask=None):
        """
        x: [batch_size, njoints, nfeats, max_frames], denoted x_t in the paper
        timesteps: [batch_size] (int)
        cmdm_text_mask: optional pre-generated CFG mask for CMDM branch [bs, 1]
        mdm_text_mask: optional pre-generated CFG mask for MDM branch [bs, 1]
        """
        if 'hint' in y.keys():
            control = self.cmdm_forward(x, timesteps, y, text_mask=cmdm_text_mask)
        else:
            if self.njoints == 205:
                n_joints = 24
            elif self.njoints == 263:
                n_joints = 22
            else:
                n_joints = 21
            y_ = {'hint': torch.zeros((x.shape[0], x.shape[-1], n_joints * 3), device=x.device)}
            y_.update(y)
            control = self.cmdm_forward(x, timesteps, y_, text_mask=cmdm_text_mask)
        output = self.mdm_forward(x, timesteps, y, control, text_mask=mdm_text_mask)
        return output
...
class InputProcess(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.data_rep in ['rot6d', 'xyz', 'hml_vec', 'smpl']:
